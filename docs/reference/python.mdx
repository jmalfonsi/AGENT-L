...
<AccordionGroup>
  ...
  <Accordion title="Jugement fermé (v1.10)">
    `LLM.judge(task, context, questions)` rend `{champ: {value, p, confidence}}`. L’implémentation par défaut traduit vers `reason()` et laisse `p` indéterminé ; un adaptateur qui sait calibrer la redéfinit (`examples/jev_llm.py`, `JevLLM`). `MockLLM({"tâche": {"kind": {"value": "question", "p": 0.94}}})` script un jugement et sa probabilité. Détail : [Jugement et oracles](/core/judgment-oracles).
  </Accordion>
