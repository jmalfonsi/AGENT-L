...
Le LLM sert aux blocs `REASON` et `JUDGE` (questions fermées, avec la probabilité de la réponse : voir [Jugement et oracles](/core/judgment-oracles)), à la sélection lorsque le programme l’autorise et à certains flux d’auteur. Ses sorties structurées ne contournent jamais :
