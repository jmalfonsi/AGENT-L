...
// --- Dictionnaire de Documentation & Tooltips ---
...
const AGENTL_DOCS: Record<string, KeywordDoc> = {
    "AGENT": {
...
function runDiagnostics(document: vscode.TextDocument) {
    ...
    const enableVerify = config.get<boolean>('enableVerify', true);
    ...
    exec(checkCmd, { cwd }, (error, stdout, stderr) => {
        ...
        if (enableVerify && !output.includes('erreur E')) {
            ...
            exec(verifyCmd, { cwd }, (vErr, vStdout, vStderr) => {
 ...
 */
export function maskNonCode(lines: string[]): string[] {
    const out: string[] = [];
    ...
    for (const raw of lines) {
        ...
        while (i < chars.length) {
            ...
            if (inBlockComment) {
                ...
                chars[i] = ' ';
                if (closing) { chars[i + 1] = ' '; i += 2; inBlockComment = false; continue; }
                i += 1;
                continue;
            }
            ...
            if (inString) {
                ...
                if (c === '\\' && next !== undefined) { chars[i] = ' '; chars[i + 1] = ' '; i += 2; continue; }
                chars[i] = ' ';
                if (c === '"') { inString = false; }
                i += 1;
                continue;
            ...
            if (c === '"') { chars[i] = ' '; inString = true; i += 1; continue; }
            if (c === '/' && next === '*') { chars[i] = ' '; chars[i + 1] = ' '; i += 2; inBlockComment = true; continue; }
            if ((c === '/' && next === '/') || c === '#') {
 ...
 * livrés.
 */
export function collectStyleIssues(source: string): StyleIssue[] {
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        const declared: Array<[number, number, string]> = [];
        ...
        const isAgentName = (start: number, length: number) =>
            declared.some(([s, l]) => s === start && l === length);
        ...
        while ((match = regex.exec(line)) !== null) {
            const word = match[0];
            if (RESERVED_KEYWORDS.has(word)) { continue; }
            if (isAgentName(match.index, word.length)) { continue; }
        ...
        for (const [start, , name] of declared) {
            if (isLowerCaseName(name)) { continue; }
            issues.push({
...
function checkReservedKeywords(document: vscode.TextDocument, diagnostics: vscode.Diagnostic[]) {
    for (const issue of collectStyleIssues(document.getText())) {
        const range = new vscode.Range(
            new vscode.Position(issue.line, issue.start),
            new vscode.Position(issue.line, issue.start + issue.length)
        );
...
function parseDiagnosticsOutput(output: string, document: vscode.TextDocument, diagnostics: vscode.Diagnostic[]) {
...
function getLineRange(document: vscode.TextDocument, lineNum: number): vscode.Range {
